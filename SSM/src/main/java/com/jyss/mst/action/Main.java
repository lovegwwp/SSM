...
public class Main {
	...
	/*
	 * // 测试系统商交易保障调度 public void test_monitor_schedule_logic() { // 启动交易保障线程 //
	 ...
	 * demoRunner.schedule();
	 * 
	 * // 启动当面付，此处每隔5秒调用一次支付接口，并且当随机数为0时交易保障线程退出 while (Math.random() != 0) {
	 * test_trade_pay(tradeWithHBService); Utils.sleep(5 * 1000); }
	 * 
	 * // 满足退出条件后可以调用shutdown优雅安全退出 demoRunner.shutdown(); }
	 */
	...
	public void test_monitor_sys() {
		...
		sysTradeInfoList.add(SysTradeInfo.newInstance("00000001", 5.2,
				HbStatus.S));
		sysTradeInfoList.add(SysTradeInfo.newInstance("00000002", 4.4,
				HbStatus.F));
		sysTradeInfoList.add(SysTradeInfo.newInstance("00000003", 11.3,
				HbStatus.P));
		sysTradeInfoList.add(SysTradeInfo.newInstance("00000004", 3.2,
				HbStatus.X));
		sysTradeInfoList.add(SysTradeInfo.newInstance("00000005", 4.1,
				HbStatus.X));
		...
		AlipayHeartbeatSynRequestBuilder builder = new AlipayHeartbeatSynRequestBuilder()
				.setAppAuthToken(appAuthToken).setProduct(Product.FP)
				.setType(Type.CR).setEquipmentId("cr1000001")
				.setEquipmentStatus(EquipStatus.NORMAL)
				.setTime(Utils.toDate(new Date())).setStoreId("store10001")
				.setMac("0a:00:27:00:00:00").setNetworkType("LAN")
		...
		MonitorHeartbeatSynResponse response = monitorService
				.heartbeatSyn(builder);
	...
	public void test_monitor_pos() {
		...
		AlipayHeartbeatSynRequestBuilder builder = new AlipayHeartbeatSynRequestBuilder()
				...
				.setNetworkStatus("gggbbbgggnnn").setNetworkType("3G")
				.setBattery("98").setWifiMac("0a:00:27:00:00:00")
	...
	public void test_trade_pay(AlipayTradeService service) {
		...
		GoodsDetail goods2 = GoodsDetail.newInstance("goods_id002", "xxx牙刷",
				500, 2);
		...
		AlipayTradePayRequestBuilder builder = new AlipayTradePayRequestBuilder()
				...
				.setAuthCode(authCode).setTotalAmount(totalAmount)
				.setStoreId(storeId)
				.setUndiscountableAmount(undiscountableAmount).setBody(body)
				.setOperatorId(operatorId).setExtendParams(extendParams)
				.setSellerId(sellerId).setGoodsDetailList(goodsDetailList)
	...
	public void test_trade_refund() {
		...
		AlipayTradeRefundRequestBuilder builder = new AlipayTradeRefundRequestBuilder()
				...
				.setRefundReason(refundReason).setOutRequestNo(outRequestNo)
				.setStoreId(storeId);
